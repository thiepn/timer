...
export class TimerDB {
  ...
  async open() {
    if (!('indexedDB' in globalThis)) {
      this.memory = { routines: new Map(), sessions: new Map(), settings: new Map(), active: new Map() };
      return this;
    }
  ...
  async all(name) {
    if (this.memory) return [...this.memory[name].values()].map(structuredClone);
    const { store } = this.store(name);
  ...
  async exportData() {
    const [routines, sessions, settings] = await Promise.all([
      this.all('routines'), this.all('sessions'), this.loadSettings()
    ]);
    return {
  ...
  async importData(data, { replace = false } = {}) {
    if (!data || data.format !== 'thiepn-timer-backup' || data.version !== 1) throw new Error('Unsupported backup format.');
    if (!Array.isArray(data.routines) || !Array.isArray(data.sessions) || !data.settings) throw new Error('Backup is incomplete.');
    if (replace) {
      await Promise.all(['routines', 'sessions'].map((s) => this.clear(s)));
    }
