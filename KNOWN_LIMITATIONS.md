...
Timer v2.4 is a local-first web/PWA application. These limitations are deliberate and are not represented as working native features:
...
The automated v2.4 release gate certifies deterministic engine behavior, persistence, backup compatibility, security/static policy, accessibility contracts, performance budgets and PWA packaging. This environment does not provide a physical Android device, Bluetooth stack or phone-call lifecycle, so real-device Bluetooth/music/call/thermal behavior is not claimed as laboratory-certified by the automated suite.
